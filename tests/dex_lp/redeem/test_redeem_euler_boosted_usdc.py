...
def test_single_maturity_full_redemption_success(StratEulerBoostedPoolUSDCPrimary):
...
def test_single_maturity_partial_redemption_success(StratEulerBoostedPoolUSDCPrimary):
    redeemParams = get_redeem_params(0, 0, get_dynamic_trade_params(
        DEX_ID["UNISWAP_V3"], TRADE_TYPE["EXACT_IN_SINGLE"], 5e6, True, get_univ3_single_data(3000)
    ))
    redeem(
...
def test_multiple_maturities_full_redemption_success(StratEulerBoostedPoolUSDCPrimary):
    redeemParams = get_redeem_params(0, 0, get_dynamic_trade_params(
        DEX_ID["UNISWAP_V3"], TRADE_TYPE["EXACT_IN_SINGLE"], 5e6, True, get_univ3_single_data(3000)
    ))
    redeem(
