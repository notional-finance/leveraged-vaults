...
contract Test_EtherFiVault is BaseAcceptanceTest {
    ...
    function deployTestVault() internal override returns (IStrategyVault) {
        IStrategyVault impl = new EtherFiVault(NOTIONAL, TRADING_MODULE);
        ...
        weETH = address(EtherFiVault(payable(address(proxy))).weETH());
        setTokenPermissions(
            address(proxy),
            weETH,
    ...
    function test_DeleverageAccount_WhenThereIsNoPreviousWithdrawRequest(uint8 maturityIndex, uint256 depositAmount) public {
        ...
        (VaultAccount memory vaultAccount) = NOTIONAL.getVaultAccount(account, address(etherFiVault));
        (VaultAccount memory liquidatorAccount) = NOTIONAL.getVaultAccount(liquidator, address(etherFiVault));
    ...
    function test_exitVaultWithoutSelling(
        ...
        vm.expectRevert();
        uint256 totalToReceiver = NOTIONAL.exitVault(account, address(vault), account, vaultShares, lendAmount, 0, "");
        ...
        vm.expectRevert();
        totalToReceiver = NOTIONAL.exitVault(account, address(vault), account, vaultShares, lendAmount, 0, "");
