...
def test_single_maturity_success(StratEulerBoostedPoolDAIPrimary):
    (env, vault, mock) = StratEulerBoostedPoolDAIPrimary
    ...
    daiBefore = env.tokens["DAI"].balanceOf(env.aaveLiquidator.owner())
    ...
    assert pytest.approx(env.tokens["DAI"].balanceOf(env.aaveLiquidator.owner()) - daiBefore, rel=5e-2) == expectedProfit