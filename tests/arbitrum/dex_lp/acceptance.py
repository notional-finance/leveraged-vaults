...
from tests.arbitrum.dex_lp.helpers import (
    snapshot_invariants, 
    check_invariants, 
    check_account,
    enterMaturity, 
...
def normal_settlement(context, depositAmount, primaryBorrowAmount, maturityIndex, depositor, operator, redeemParams, percent):
    ...
    vaultState = env.notional.getVaultState(vault.address, maturity)
    totalUnderlyingCash = 0 # convert_to_underlying(env, currencyId, vaultState["totalAssetCash"], context.primaryPrecision)
